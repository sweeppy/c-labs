...
short *day_of_month(unsigned short day, unsigned short year)
{
    ...
    dayAndMonth[0] = day;
    dayAndMonth[1] = ++month; // так как счет месяца начинается с 0 (для правильной индексации nDatTab)
    return dayAndMonth;
}

// TODO: почему arr передается и по указателю, и по ссылке?
// TODO: убрать, раз не стоит
void add_unique(int *&arr, int &length, int newValue)
{
    ...
    delete[] arr; // из-за освобождения памяти по ссылке
    arr = newArr;
}
